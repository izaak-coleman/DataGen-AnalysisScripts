...
def get_valid_ranges(chromosome, n_clusters, fa_len, bed_path, fasta):
  random.seed()
  ranges = []
  while len(ranges) != n_clusters:
    r = random.randint(0, fa_len-501)
    r = (r, r+500)
...
def main():
  if len(sys.argv) != 6:
    ...
    sys.exit(1)
  p_basename = sys.argv[1]
  p_basename = p_basename[p_basename.rfind('/')+1:]
  m_basename = sys.argv[2]
  m_basename = m_basename[m_basename.rfind('/')+1:]

  chromosome, p_fasta = load_fasta(sys.argv[1])
  _, m_fasta = load_fasta(sys.argv[2])
  ranges = get_valid_ranges(chromosome, int(sys.argv[3]), len(p_fasta), sys.argv[4], m_fasta)
  add_clusters(chromosome, p_fasta, p_basename[:-3], ranges[:len(ranges)/2], int(sys.argv[5]))
  add_clusters(chromosome, m_fasta, m_basename[:-3], ranges[len(ranges)/2:], int(sys.argv[5]))
