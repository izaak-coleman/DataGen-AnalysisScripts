...
def calculate_recall(calls, snvs):
  snvs_set = set(snvs)
  intersection = [call for call in calls if call in snvs_set] # get intersection
  return float(len(intersection)) / float(len(snvs))
...
def main():
  ...
  print_tuples(calc_snv_intersection(calls, snvs))
  print("Recall: " + str(calculate_recall(calls, snvs)))
